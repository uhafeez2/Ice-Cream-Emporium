...
//         CREATE MANAGER
 ...
 //         COSTS REPORT - EXPENSES/ PROFIT LOSS STATEMENT/ ETC....
    // Append to the Role menu
    Gtk::MenuItem *menuitem_costs_report = Gtk::manage(new Gtk::MenuItem("_Expenses", true));
    menuitem_costs_report->signal_activate().connect(sigc::mem_fun(*this, &Mainwin::on_costs_click));
    reportmenu->append(*menuitem_costs_report);

   //           INVENTORY CLICK
    // Append to the Role menu
    Gtk::MenuItem *menuitem_inventory_report = Gtk::manage(new Gtk::MenuItem("_Inventory", true));
    menuitem_inventory_report->signal_activate().connect(sigc::mem_fun(*this, &Mainwin::on_inventory_click));
