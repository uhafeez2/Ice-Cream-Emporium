...
void Mainwin::on_create_order_click() {
 ...
 on_display_costs_click(_emp->num_orders()-1);
...
void Mainwin::on_costs_click() {
   on_display_costs_click(select_order());
}
...
void Mainwin::on_display_costs_click(int order) {
    // Data validation!
    if (0 > order || order >= _emp->num_orders()) return;
...
//inventory click --display inventory***
void Mainwin::on_inventory_click() {
    on_display_server_prep_click(select_order());
}

//****display inventory
void Mainwin::on_display_inventory_click(int order) {
    // Data validation!
    if (0 > order || order >= _emp->num_orders()) return;
